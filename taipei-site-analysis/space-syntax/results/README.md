...
道路灰、步行綠、天橋藍；未繪製建築。外緣用來源寬度與製圖寬度推定，缺步道寬度採2m，不是現勘測量。道路來源寬度可能已包含人行道，不可直接拿圖面分區做實測面積統計。合併面域只用於繪圖，不會新增跨路或跨層接線。

網頁切換起點與400/800/1600m門檻，標記可達的南側目的點及其最短路徑；不是完整等時圈。匯出按鈕下載目前分析畫面的SVG。A.svg/B.svg保留八組同編號路徑靜態圖。

已檢查封閉路徑、路由數值不變、24種起點門檻選擇的48組A/B計數與分析圖層開關（DOM邏輯測試，非瀏覽器截圖驗證）。
