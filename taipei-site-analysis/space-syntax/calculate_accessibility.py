...
D=Path(__file__).parent;OUT=D/'results';OUT.mkdir(exist_ok=True)
...
# Fixed ground OD sample: nearest mapped ground node to 8 north/south corridor targets.
axis0=project(121.5103681,25.0501216);axis1=project(121.5184222,25.0483928)
dx,dy=axis1[0]-axis0[0],axis1[1]-axis0[1];L=math.hypot(dx,dy);nx,ny=-dy/L,dx/L
samples=[];used=set()
for i in range(8):
 f=(i+.5)/8;cx,cy=axis0[0]+dx*f,axis0[1]+dy*f
 for side,sgn in [('N',1),('S',-1)]:
  target=(cx+sgn*nx*120,cy+sgn*ny*120)
  candidates=[k for k in surface_nodes if k in A and k not in used and ((positions[k][0]-axis0[0])*nx+(positions[k][1]-axis0[1])*ny)*sgn>30]
  k=min(candidates,key=lambda k:(math.dist(target,positions[k]),k))
  offset=math.dist(target,positions[k]);used.add(k);samples.append(dict(id=side+str(i+1),node=k,side=side,target_offset_m=round(offset,2),sample_valid=offset<=150,lon=ll[k[0]][0],lat=ll[k[0]][1]))
samples=[s for s in samples if s['sample_valid']]
...
for case,g in [('A',A),('B',B)]:
 ...
 for e in ways:
  t=e.get('tags',{});geo=e.get('geometry',[])
  if t.get('building') and geo and any(121.5088<p['lon']<121.5203 and 25.0447<p['lat']<25.052 for p in geo):svg.append('<polygon points="'+line([project(p['lon'],p['lat']) for p in geo])+'" fill="#d9dcd8"/>')
