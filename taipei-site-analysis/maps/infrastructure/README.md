...
2026-09-15 現行版本：`index.html` 顯示市民大道全段純圖塊版，讀取 `full-corridor/` 三張 SVG。請保留該子資料夾。無可見文字，平面道路填白。重建使用 `full-corridor/build_full.py`；範圍、比例與限制見 `full-corridor/SOURCE_NOTES.md`。

以下為 2026-09-14 西段版本的留存說明；執行舊 build_page.py 會重建舊版頁面，若要回到全段版請執行 build_full.py。各張 SVG 可單獨編輯。圖框 1,980 × 1,130 m；420 × 297 mm，100% 列印 1:5,000。列印縮放後請以比例尺判讀。
